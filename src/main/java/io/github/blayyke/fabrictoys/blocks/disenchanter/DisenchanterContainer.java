...
public class DisenchanterContainer extends Container {
    ...
    private final DisenchanterBlockEntity disenchanter;
    private final BlockContext context;

    public DisenchanterContainer(int syncId, PlayerEntity player, BlockContext context) {
        super(null, syncId);
        this.player = player;
        ...
        this.disenchanter = (DisenchanterBlockEntity) blockEntity;
        int xOffset = 35;
        int yOffset = 20;

        // Blank disc slot
        addSlot(new Slot(disenchanter, 0, xOffset + (1 * 18), yOffset) {
            @Override
            public boolean canInsert(ItemStack itemStack_1) {
                return itemStack_1.getItem() instanceof BookItem;
            }
        });

        // Disc slot
        addSlot(new Slot(disenchanter, 1, xOffset + (2 * 18), yOffset) {
//            @Override
//            public boolean canInsert(ItemStack itemStack_1) {
...
//            }
        ...
        // Output slot
        addSlot(new Slot(disenchanter, 2, xOffset + (4 * 18), yOffset) {
            @Override
            public boolean canInsert(ItemStack itemStack_1) {
                return false;
            }
    ...
    public boolean canUse(PlayerEntity player) {
        return canUse(this.context, player, FTBlocks.DISENCHANTER);
    }
